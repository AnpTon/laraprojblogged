<x-app-layout>
                            ...
                            <div id="selected-tags" class="flex flex-wrap gap-2 mt-2">
                                @foreach($post->tags as $tag)
                                    <span class="inline-flex items-center px-2 py-1 bg-indigo-100 text-indigo-800 rounded text-sm">
                                        #{{ $tag->name }}
                                        <button type="button" class="ml-1 text-indigo-600 hover:text-indigo-900" onclick="removeTag('{{ $tag->name }}')">&times;</button>
                                    </span>
                                @endforeach
                            </div>
                            <input type="hidden" name="tags" id="tags-hidden" value="{{ json_encode($post->tags->pluck('name')) }}">
                        </div>
    ...
    <script>
        const selectedTags = {{ json_encode($post->tags->pluck('name')) }};
